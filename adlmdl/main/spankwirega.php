...
//********************************************************************************************************************************************************************
	if ($_GET['search']<>''){
header("Location: ".$mdldzn.$dosya.'?nedir=filmler&filmler='.$site."/search/gay/keyword/".$_GET['search']);
	}
//********************************************************************************************************************************************************************
	...
	$echo.="<?xml version=\"1.0\" encoding=\"utf-8\" ?>
		<items>
		<playlist_name><![CDATA[GAY KATEGORİLER]]></playlist_name>
		"; 
		...
		if (($_SERVER['HTTP_USER_AGENT']=='') or ($_SERVER['HTTP_USER_AGENT']=='Mozilla/5.0 seyirTURKVOD 0.1')){
			$echo.=playlist('ARAMA',$pic,'LÜTFEN BİR SONRAKİ EKRANDA KLAVYEYİ KULLANABİLMEK İÇİN STOP TUŞUNA BASINIZ.','seyirTURKModul@webot@'.substr($site,7).$dosya.'?nedir=filmler&filmler='.$site.'search/gay/keyword/@ARAMA','playlist');
...
//********************************************************************************************************************************************************************
	...
	$echo.="<?xml version=\"1.0\" encoding=\"utf-8\" ?>
		...
		foreach ($kategoriler[1] as  $key => $kategori ) {
			...
			$link=$mdldzn.$dosya.'?nedir=filmler&filmler=http://www.spankwire.com'.$kategoriler[1][$key].'&before=?nedir=kategoriler$kategoriler='.$_GET['kategoriler'];
			$echo.=playlist($name,$pic,$desc1,$link.'&xml','playlist');
			}
		$echo.=playlist('ÖNCEKİ MENÜ',$prevmenupic,$desc1,$mdldzn.$dosya.'?nedir=anakate'.'&xml','playlist');
